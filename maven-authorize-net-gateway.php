...
/*
  Plugin Name: Maven Authorize.net Gateway
  Plugin URI:
  Description:
  Author: Site Mavens
  Version: 0.1
...
namespace MavenAuthorizeNetGateway;
 ...
 */
class AuthorizeNetGateway extends \Maven\Gateways\Gateway {
	...
	public function __construct() {
		...
		$transactionTypeOptions  = array(
			array('id'=>'CAPTURE_ONLY' , 'name'=> 'Capture Only'),
			array('id'=>'AUTH_ONLY' , 'name'=> 'Authorization Only'),
			array('id'=>'AUTH_CAPTURE' , 'name'=> 'Authorization and Capture')
		);
		
		$authorizationType = new Option("authorizationType", "Authorization Type", '', '', OptionType::DropDown);
		$authorizationType->setOptions( $transactionTypeOptions );
		
		$authorizationTypeTest = new Option("authorizationTypeTest", "Authorization Type Type", '', '', OptionType::DropDown);
		$authorizationTypeTest->setOptions( $transactionTypeOptions );
		...
		$defaultOptions = array(
			...
			new Option(
					"sendEmailToCustomerFromAuth", "Send email to customer from Authorize.net", false, '', OptionType::CheckBox
			),
			new Option(
					"delimiter",
					"Delimiter",
	...
	public function getFullUrl() {
		...
		$fields = array(
			'customer_ip' => \Maven\Core\Request::current()->getIp(),
			'delim_char' => "|",
			'version' => "3.1",
...
$authorizeNetGateway = new AuthorizeNetGateway();
\Maven\Core\HookManager::instance()->addFilter( 'maven/gateways/register', array( $authorizeNetGateway, 'register' ) );
